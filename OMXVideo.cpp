...
extern bool omxplayer_remap_wanted(void);
...
bool COMXVideo::Open(COMXStreamInfo &hints, OMXClock *clock, PwTileMap *tilemap, const CRect &DestRect, bool dump_info, float display_aspect, bool deinterlace, bool hdmi_clock_sync)
{
  OMX_ERRORTYPE omx_err   = OMX_ErrorNone;
  ...
  if (m_tilemap != NULL) {
    PwIntRect picture;
    PWRECT_SET0(picture, m_decoded_width, m_decoded_height);
    SetVideoTile(m_tilemap, &picture);
  } else {
  ...
  if (pData || iSize > 0)
  {
    ...
    while(demuxer_bytes)
    {
      ...
      */
      ...
      // Check if retiling requested
      if (m_tilemap != NULL && omxplayer_remap_wanted()) {
	PwIntRect picture;
	PWRECT_SET0(picture, m_decoded_width, m_decoded_height);
	SetVideoTile(m_tilemap, &picture);
      }
...
void COMXVideo::SetVideoTile(PwTileMap *tilemap, const PwIntRect *picture)
{
  PwIntRect src, dest;
  PwVcTransform transform;
  GError *error = NULL;

  if (! pwtilemap_map_picture(tilemap, picture,
			      &src, &dest, &transform, &error)) {
    // FIXME
    return;
  }
  ...
  // Bitmap for which aspects we are configuring
  configDisplay.set = (OMX_DISPLAYSETTYPE)(OMX_DISPLAY_SET_FULLSCREEN |
					   OMX_DISPLAY_SET_NOASPECT |
					   OMX_DISPLAY_SET_DEST_RECT |
					   OMX_DISPLAY_SET_SRC_RECT |
					   OMX_DISPLAY_SET_TRANSFORM);

  // We manage scaling ourselves
  configDisplay.fullscreen = OMX_FALSE;
  configDisplay.noaspect   = OMX_TRUE;

  configDisplay.dest_rect.x_offset  = dest.x0;
  configDisplay.dest_rect.y_offset  = dest.y0;
  configDisplay.dest_rect.width     = PWRECT_WIDTH(dest);
  configDisplay.dest_rect.height    = PWRECT_HEIGHT(dest);

  configDisplay.src_rect.x_offset   = src.x0;
  configDisplay.src_rect.y_offset   = src.y0;
  configDisplay.src_rect.width      = PWRECT_WIDTH(src);
  configDisplay.src_rect.height     = PWRECT_HEIGHT(src);

  // Assume PwTransform has same values as OMX_DISPLAYTRANSFORMTYPE
